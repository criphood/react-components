describe('e2e test', () => {
  ...
  it('main page', () => {
    cy.visit('/');
    cy.get('input[placeholder="Search..."]').type('car{enter}').should('have.value', 'car');
  ...
  it('forms', () => {
    cy.visit('/forms');
    cy.get('#name').type('Artem').should('have.value', 'Artem');
    cy.get('#birthday').type('2018-07-22').should('have.value', '2018-07-22');
    cy.get('#city').click();
    cy.get('li[data-value="Moscow"]').click();
    cy.get('input[type="radio"]').eq(0).click();
    cy.get('.avatar__label').click();
    cy.get('#avatar').attachFile('test.jpg');
    cy.get('#consent').click().should('be.checked');
